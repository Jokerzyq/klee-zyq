...
//cyj: Determine whether the given query expression is Valid given the constraints
...
bool TimingSolver::mustBeTrue(const ExecutionState& state, ref<Expr> expr, 
  ...
  // Fast path, to avoid timer and OS overhead.
  if (ConstantExpr *CE = dyn_cast<ConstantExpr>(expr)) {
    result = CE->isTrue() ? true : false;	 //cyj: Is this the true expression.
    return true;
  }
  ...
  if (simplifyExprs)
    expr = state.constraints.simplifyExpr(expr);		//cyj: constraints: Constraints collected so far
  ...
  delta -= now;
  stats::solverTime += delta.usec();
  state.queryCost += delta.usec()/1000000.;//cyj: Costs for all queries issued for this state, in seconds
...
bool TimingSolver::getValue(const ExecutionState& state, ref<Expr> expr, 
  ...
  bool success = solver->getValue(Query(state.constraints, expr), result);	//cyj: true
...
//cyj: 求解器计算当前state下各符号化变量的初始值, 统计求解开销/查询开销
//cyj: seed模式下该步会不会不同?
bool 
