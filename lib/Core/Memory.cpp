...
const UpdateList &ObjectState::getUpdates() const {
  ...
  if (!updates.root) {
    ...
    std::vector< std::pair< ref<Expr>, ref<Expr> > > Writes(NumWrites);	//cyj: 记录了每个UpdateNode中的index与value信息
    const UpdateNode *un = updates.head;
    for (unsigned i = NumWrites; i != 0; un = un->next) {
      --i;
      Writes[i] = std::make_pair(un->index, un->value);	//cyj: 之前的信息在前面,更新的信息在后面
    }
    ...
    for (; Begin != End; ++Begin) {
      ...
      Contents[Index->getZExtValue()] = Value;	//cyj: Contents中的内容全是concrete
    }
    ...
    /*//cyj add--------------------------------------
    llvm::errs() << "Array Info: --------------------------------------------------\n";
    llvm::errs() << "Array name: " << array->name << "\n";
    llvm::errs() << "Array size: " << array->size << "\n";
    llvm::errs() << "Array constantValues:\n";
    for(std::vector<ref<ConstantExpr> >::const_iterator bi = array->constantValues.begin(), ei = array->constantValues.end();
    			bi != ei; bi++){
    	(*bi)->dump();
    }
    llvm::errs() << "end Array constantValues. --------------------------------------------\n";
    //add end--------------------------------------*/
...
void ObjectState::flushRangeForWrite(unsigned rangeBase, 
  ...
  for (unsigned offset=rangeBase; offset<rangeBase+rangeSize; offset++) {
    if (!isByteFlushed(offset)) {	//cyj: 将每一个byte上的具体值或符号值加入到updates列表, 每一个byte的flushMask置0
      if (isByteConcrete(offset)) {
        updates.extend(ConstantExpr::create(offset, Expr::Int32),	//cyj: head = new UpdateNode
                       ConstantExpr::create(concreteStore[offset], Expr::Int8));
...
ref<Expr> ObjectState::read8(unsigned offset) const {
  if (isByteConcrete(offset)) {
    ...
    /*//cyj add----------------------------------------------------------
    ref<Expr> refExpr = ReadExpr::create(getUpdates(), ConstantExpr::create(offset, Expr::Int32));
    refExpr->dump();
    return refExpr;
    //add end---------------------------------------------------------*/
  }
...
ref<Expr> ObjectState::read8(ref<Expr> offset) const {
  ...
  flushRangeForRead(base, size);	//cyj: updates里加入新的内容, 更新updates
  ...
  /*//cyj add
  ref<Expr> refExpr = ReadExpr::create(getUpdates(), ZExtExpr::create(offset, Expr::Int32));
  refExpr->dump();
  return refExpr;
  //add end*/
}

void ObjectState::write8(unsigned offset, uint8_t value) { //cyj: typedef unsigned char uint8_t;
...
void ObjectState::write8(ref<Expr> offset, ref<Expr> value) {
  ...
  unsigned base, size;
  fastRangeCheckOffset(offset, &base, &size);	//cyj: base = 0; size = ObjectState.size
...
//cyj: ObjectState.concreteStore写为Value, setKnownSymbolic写为0
