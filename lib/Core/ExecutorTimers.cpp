...
static void setupHandler() {
  ...
  ::setitimer(ITIMER_REAL, &t, 0);		 //cyj: 实现精度较高的定时功能
...
//cyj: 生成ptree(instructions).dot文件或states.txt文件, 并写入内容
...
void Executor::processTimers(ExecutionState *current,
  ...
  if (ticks || dumpPTree || dumpStates) {
	//cyj: 生成ptree(instructions).dot文件, 并写入内容
	if (dumpPTree) {
      char name[32];
      sprintf(name, "ptree%08d.dot", (int) stats::instructions);
      	  	//cyj: 在printf中%08d表示输出宽度为8，不足8位的补0，%8d表示输出宽度为8，不足8位的补空格
      llvm::raw_ostream *os = interpreterHandler->openOutputFile(name);
      if (os) {
        processTree->dump(*os);		//cyj: dump(): 给os流写入字符串
        delete os;
      }
    ...
    //cyj: 如果超过maxInstTime, 终止当前ExecutorState
    ...
    //cyj: 为Executor.timers设置nextFireTime
    if (!timers.empty()) {
