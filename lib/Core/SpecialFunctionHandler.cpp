...
//cyj: 把存在的特殊函数绑定在handlers中. std::map<const llvm::Function*, std::pair<Handler,bool> >
//cyj: Function就是klee_make_symbolic(); Handler就是handleMakeSymbolic()
...
//cyj:查找f是否是specialFunction, 如果是的话调用SpecialFunctionHandler::*Handler, 比如handleMakeSymbolic
bool SpecialFunctionHandler::handle(ExecutionState &state, 
                                    Function *f,
                                    KInstruction *target,
                                    std::vector< ref<Expr> > &arguments) {
  handlers_ty::iterator it = handlers.find(f);		//cyj: 查找f是否是specialFunction
  if (it != handlers.end()) {    
    ...
    if (!hasReturnValue && !target->inst->use_empty()) {
      ...
      (this->*h)(state, target, arguments);	//cyj: 调用SpecialFunctionHandler::*Handler, 比如handleMakeSymbolic
    }
    return true;
  } else {
    return false;
...
/****/
//cyj: 找到addressExpr所在的pair<MemoryObject*, ObjectState*>, 把ObjectState中的conc或symb内容读出来
//cyj: 疑问: 什么时候符号化变量的MemoryObject与ObjectState捆绑到一起??
// reads a concrete string from memory
std::string 
SpecialFunctionHandler::readStringAtAddress(ExecutionState &state, 
                                            ref<Expr> addressExpr) {
  ObjectPair op;	//cyj: std::pair<const MemoryObject*, const ObjectState*>
  addressExpr = executor.toUnique(state, addressExpr);
  	  	  	  	  	  	  	  	  //cyj: Compute a feasible value for the expression
  ref<ConstantExpr> address = cast<ConstantExpr>(addressExpr);
  if (!state.addressSpace.resolveOne(address, op))
	  	  	  	  	  	  	  	  //cyj: op为一个pair<const MemoryObject*, ObjectHolder>
...
//cyj: 通过arguments[2]得到name; 通过arguments[0]得到符号化变量所属的MemoryObject,
//cyj: 把若干符号化变量所属的pair(mo/os)与state对应起来, 并进行符号化.
void SpecialFunctionHandler::handleMakeSymbolic(ExecutionState &state,
  ...
  // correct arguments.
  if (arguments.size() == 2) {	//cyj: arguments就是klee_make_symbolic()的参数
    name = "unnamed";
  } else {
    ...
    //cyj: 找到arguments[2]存的地址所在的pair<MemoryObject*, ObjectState*>, 把ObjectState中的string读出来
    //cyj: 就是读arguments[2]所指的string? 这么费劲绕一圈干吗
  }

  Executor::ExactResolutionList rl;
  //cyj: std::vector< std::pair<std::pair<const MemoryObject*, const ObjectState*>, ExecutionState*> >
  executor.resolveExact(state, arguments[0], rl, "make_symbolic");
  
  //cyj: 对ExactResolutionList中所记录的每个state/mo/os对进行符号化
  for (Executor::ExactResolutionList::iterator it = rl.begin(), 
         ie = rl.end(); it != ie; ++it) {
    ...
    if (res) {
      executor.executeMakeSymbolic(*s, mo, name);	//cyj: 进行符号化
    } else {      
      executor.terminateStateOnError(*s, 
