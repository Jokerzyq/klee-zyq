...
void StatsTracker::stepInstruction(ExecutionState &es) {
  if (OutputIStats) {
	...
	if (TrackInstructionTime) {
      ...
      } else {		//cyj: 当前时间, 用户使用时间(non-kernel), 系统使用时间(kernel)
...
///
//cyj: 对ExecutionState的最后一个StackFrame进行配置
/* Should be called _after_ the es->pushFrame() */
void StatsTracker::framePushed(ExecutionState &es, StackFrame *parentFrame) {
  if (OutputIStats) {
    StackFrame &sf = es.stack.back();		//cyj: Stack representing the current instruction stream

    if (UseCallPaths) {		//cyj: Enable calltree tracking for instruction level statistics
    ...
    if (updateMinDistToUncovered) {
      ...
      if (parentFrame)
        minDistAtRA = parentFrame->minDistToUncoveredOnReturn;
      	  	  //cyj: Minimum distance to an uncovered instruction once the function returns.
