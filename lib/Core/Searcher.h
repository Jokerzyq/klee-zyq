...
namespace klee {
  ...
  public:
    ...
    void addState(ExecutionState *es, ExecutionState *current = 0) {
      ...
      tmp.insert(es);
      update(current, tmp, std::set<ExecutionState*>());												//removed states are empty.
    }

    void removeState(ExecutionState *es, ExecutionState *current = 0) {
      std::set<ExecutionState*> tmp;
      tmp.insert(es);
      update(current, std::set<ExecutionState*>(), tmp);												//added states are empty.
    }
  ...
  class NQCSSearcher : public Searcher {																//added by ljd
      std::vector<ExecutionState*> states;
      std::vector<ExecutionState*> tempStates;
    ...
    //added by qmh
    class ToySearcher : public Searcher {
	  	Executor &executor;
  		std::vector<ExecutionState*> states;
  		std::vector<ExecutionState*> fpstates;


  	public:
  		ToySearcher(Executor &_executor);
  		~ToySearcher();
  		ExecutionState &selectState();
  	    void update(ExecutionState *current,
  	                const std::set<ExecutionState*> &addedStates,
