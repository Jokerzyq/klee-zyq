...
///
//cyj: 将MemoryObject与ObjectState绑定到一个pair
...
/// 
//cyj: 根据address<ConstantExpr>查找其所对应的MemoryObject;
//cyj: 结果保存在result中,为一个pair<const MemoryObject*, ObjectHolder>
...
bool AddressSpace::resolveOne(ExecutionState &state,
  ...
  if (ConstantExpr *CE = dyn_cast<ConstantExpr>(address)) {
    ...
    if (!solver->getValue(state, address, cex)) //cyj: getValue = true
      return false;
    ...
    while (oi!=begin) {
      ...
      if (!solver->mayBeTrue(state, 
                             mo->getBoundsCheckPointer(address), mayBeTrue))	//cyj: mayBeTrue = true
        return false;
      if (mayBeTrue) {
        result = *oi;
        success = true;
...
bool AddressSpace::resolve(ExecutionState &state,
  ...
  if (ConstantExpr *CE = dyn_cast<ConstantExpr>(p)) {
    ObjectPair res;
    if (resolveOne(CE, res))	//cyj: 根据CE(是一个地址)查找其所对应的MemoryObject;
      rl.push_back(res);
    return false;
  } else {
    TimerStatIncrementer timer(stats::resolveTime);
    uint64_t timeout_us = (uint64_t) (timeout*1000000.); //cyj: 微秒 6次方
    ...
    MemoryMap::iterator oi = objects.upper_bound(&hack);//cyj: 返回键值>hack的第一个元素
    MemoryMap::iterator begin = objects.begin();
    MemoryMap::iterator end = objects.end();
    ...
    while (oi!=begin) {
      --oi;
      const MemoryObject *mo = oi->first;
      if (timeout_us && timeout_us < timer.check())	//cyj: ??
        return true;

      // XXX I think there is some query wasteage here?
      ref<Expr> inBounds = mo->getBoundsCheckPointer(p);	//cyj: ??
      bool mayBeTrue;
