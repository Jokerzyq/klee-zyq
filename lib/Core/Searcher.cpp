...
namespace {
  ...
  cl::opt<bool>
  AllowEliminateStates("allow-eliminate-states",
            cl::desc("allow searcher eliminate 'less important states' (default = off)"),
            cl::init(false));
}
...
ExecutionState &DFSSearcher::selectState() {
  return *states.back();																		//states:vector<ExecutionState*>
}
...
ExecutionState &BFSSearcher::selectState() {
	//std::cout << "BFS states numbers:" << states.size() << std::endl;
  return *states.front();																		//states:deque<ExecutionState*>
}
...
ExecutionState &RandomSearcher::selectState() {
  ...
  return *states[theRNG.getInt32()%states.size()];												//states:vector<ExecutionState*>
}
...
WeightedRandomSearcher::WeightedRandomSearcher(WeightType _type)
  : states(new DiscretePDF<ExecutionState*>()),													//*states:DiscretePDF<ExecutionState*>;
    type(_type) {																				//type:WeightType
  switch(type) {
  case Depth: 
    updateWeights = false;
    break;
...
ExecutionState &WeightedRandomSearcher::selectState() {
  return *states->choose(theRNG.getDoubleL());								//getDoubleL():generates a random number on [0,1)-real-interval
}
...
ExecutionState &RandomPathSearcher::selectState() {
  ...
  while (!n->data) {																//reach the leaf node
    if (!n->left) {																	//is left child tree is null
      n = n->right;
    } else if (!n->right) {															//is right child tree is null
      n = n->left;
    } else {																		//is left child tree and right child tree are neither null
      if (bits==0) {
...
entry:
  ...
  if (baseSearcher->empty()) {
    std::map<llvm::Instruction*, ExecutionState*>::iterator it = 
    		statesAtMerge.begin();														//statesAtMerge:map<llvm::Instruction*, ExecutionState*>
  ...
  if (Instruction *mp = getMergePoint(es)) {											//es.pc->inst
    std::map<llvm::Instruction*, ExecutionState*>::iterator it = 
...
ExecutionState &InterleavedSearcher::selectState() {							//select a searcher circularly 0,searchers.size-1,...,0,...
...
static bool compare(const ExecutionState *a, const ExecutionState *b){
	return a->priority < b->priority;
}

ToySearcher::ToySearcher(Executor &_executor):executor(_executor){}
ToySearcher::~ToySearcher(){}

ExecutionState &ToySearcher::selectState() {
	//std::cout << fpklee::fpklee << std::endl;

	int index=0;

	if(!fpstates.empty()){
		 index = theRNG.getInt32()%fpstates.size();
	}
	...
	if(print_info){
		...
		std::cout << "number of fpstates:" << fpstates.size() << std::endl;

		int number=0;
		std::cout << "states info:" << std::endl;
		for(std::vector<ExecutionState*>::const_iterator it=states.begin(), ie=states.end(); it!=ie; it++){
				std::cout << "(" << ++number << ")" << "[" << (*it)->currentFrame.first
													<< " <- " << (*it)->currentFrame.second
													<< " == " << (*it)->priority
													//<< "," << (*it)->isInSoftFloat
													<< "," << (*it)->lib_id
													<< "," << (*it)->pc->inst->getOpcodeName()
													<< "]"<< "\t" ;
		}
		std::cout << std::endl;
		...
		number=0;
		if(!fpstates.empty()){
			std::cout << "fpstates info:" << std::endl;
			for(std::vector<ExecutionState*>::const_iterator it=fpstates.begin(), ie=fpstates.end(); it!=ie; it++){
					std::cout << "(" << ++number << ")" << "[" << (*it)->currentFrame.first
														<< " <- " << (*it)->currentFrame.second
														<< " == " << (*it)->priority
														//<< "," << (*it)->isInSoftFloat
		...
		if(fpstates.empty()){
			std::cout << "selected state:" << "[" << states.back()->currentFrame.first
										   << " <- " << states.back()->currentFrame.second
										   << " == " << states.back()->priority
										   //<< "," << states.back()->isInSoftFloat
										   << "," << states.back()->lib_id
										   << "," << states.back()->pc->inst->getOpcodeName()
										   << "]" << std::endl;
		}else{
			std::cout << "selected fpstate:" << "(" << index+1 << ") "
											 << "[" << fpstates[index]->currentFrame.first
											 << " <- " << fpstates[index]->currentFrame.second
											 << " == " << fpstates[index]->priority
											 //<< "," << fpstates[index]->isInSoftFloat
											 << ","<< fpstates[index]->lib_id
										 	 << "," << fpstates[index]->pc->inst->getOpcodeName()
											 << "]"<< std::endl;
		}

		std::cout << "hotspot == " << hotspot_id << ";\t"
				  << "ret_true == " << ret_true << ";\t"
				  << "ret_false == " << ret_false << ";\t"
				  << "ret_ok == "<< ret_ok << ";\t"
				  << "fpEnough == " << fpEnough;
	...
	if(fpstates.empty()){
		...
		//if(fpstates.back())std::cout << "fpstates.back()" << std::endl;
		return *fpstates[index];
		//*fpstates.back();
	}
}

void ToySearcher::update(ExecutionState *current,
                         const std::set<ExecutionState*> &addedStates,
                         const std::set<ExecutionState*> &removedStates) {
  states.insert(states.end(),
                addedStates.begin(),
  ...
  if(!states.empty()){
	...
	for(std::vector<ExecutionState*>::const_iterator it = states.begin(), ie=states.end(); it!=ie; it++){
		(*it)->updateFrame();
		if((*it)->priority >= 0){
			if((*it)->lib_id == 0){//the state is in source code
				(*it)->priority = 0;
			}else{//the state is in FloatingPoint lib code
				if((*it)->lib_id != hotspot_id){
					(*it)->priority = 1;
				}else{
					if(!fpEnough){
						(*it)->priority = 2;
	...
	//sort states by priority in ascending order
	sort(states.begin(), states.end(), compare);

	//allow searcher to eliminate "less important states"
	if(AllowEliminateStates){
		for(std::vector<ExecutionState*>::iterator it=states.begin(), ie=states.end(); it!=ie; it++){
			if((*it)->priority < 0){
				executor.removedStates.insert(*it);
			}
		}
	}
	...
	//copy the hotspot references to fpstates
	fpstates.clear();
	for(std::vector<ExecutionState*>::const_iterator it=states.begin(),
			ie = states.end(); it!=ie; ++it){
		  if((*it)->priority > 0 && (*it)->lib_id == hotspot_id){
			  fpstates.push_back(*it);
	  }
	}

	//change hotspot and copy the references to fpstates;
	if(fpstates.empty()){
		ret_true = 0;
		ret_false = 0;
		ret_ok = false;
		...
		fpEnough=false;
		if(current){
			//std::cout << "current->stack.back().frame_id == " << current->stack.back().frame_id << std::endl;
			hotspot_id = current->stack.back().frame_id;
		}
		for(std::vector<ExecutionState*>::const_iterator it=states.begin(),
				ie = states.end(); it!=ie; ++it){
