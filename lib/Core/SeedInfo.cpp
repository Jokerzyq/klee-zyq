...
KTestObject *SeedInfo::getNextInput(const MemoryObject *mo,
  ...
  if (byName) {
    ...
    //cyj: 如果KTestObject名字与mo名字匹配, 返回该KTestObject
    for (i=0; i<input->numObjects; ++i) {
      KTestObject *obj = &input->objects[i];
      if (std::string(obj->name) == mo->name)
        if (used.insert(obj).second)
          //cyj: 已经有这个元素,则插入失败. 插入成功则set.insert.second返回1，否则返回0
          return obj;
    }
    
    //cyj: 如果第一个没有被使用过的KTestObject的大小与mo大小一致, 则返回该KTestObject
    // If first unused input matches in size then accept that as well.
    for (i=0; i<input->numObjects; ++i)
    ...
    return 0;
  } else {	//cyj: 如果不使用name匹配KTestObject, 则返回第inputPosition个KTestObject
