...
ref<Expr> ExprVisitor::visit(const ref<Expr> &e) {
  if (!UseVisitorHash || isa<ConstantExpr>(e)) {
    return visitActual(e);	//cyj: 对expr进行处理, 输出符合要求的格式(如:expr为ConstantExpr)
  } else {
    ...
    if (it!=visited.end()) {
      ...
      ref<Expr> res = visitActual(e);	//cyj: 对expr进行处理, 输出符合要求的格式(如:expr为ConstantExpr)
...
ref<Expr> ExprVisitor::visitActual(const ref<Expr> &e) {
  if (isa<ConstantExpr>(e)) {    
    ...
    Expr &ep = *e.get();	//cyj: get(): 得到指向Expr的指针

    Action res = visitExpr(ep);		//cyj: return Action::doChildren();
    switch(res.kind) {
