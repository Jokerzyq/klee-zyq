...
SMTParser::SMTParser(const std::string _filename, 
  ...
  varEnvs.push(VarEnv());		//cyj: std::stack<map<string, ExprHandle>> varEnvs
  fvarEnvs.push(FVarEnv());			//cyj: std::stack<map<string, ExprHandle>> FvarEnvs
}
...
bool SMTParser::Solve() {
  ...
  if (UseSTPQueryPCLog)
    S = createPCLoggingSolver(S, "stp-queries.pc");		//cyj: 两个参数?
  if (UseFastCexSolver)
  ...
  Decl *D = this->ParseTopLevelDecl();		//cyj: Decl - Base class for top level declarations.
