...
//qmh add----------------------------------------------------------------------------------------------------------------------------------
Module *klee::linkWithFPLibrary(Module *module,
                              const std::string &libraryName) {
  KLEE_DEBUG_WITH_TYPE("klee_linker", dbgs() << "Linking file " << libraryName << "\n");
#if LLVM_VERSION_CODE >= LLVM_VERSION(3, 3)
  if (!sys::fs::exists(libraryName)) {
  ...
  if (magic == sys::fs::file_magic::bitcode) {
    ...
    fpFunctions.clear();
    fpStapleFunctions.clear();
    fpCmpFunctions.clear();
    for(Module::iterator it=Result->begin(),ie=Result->end(); it!=ie; it++){
    	fpFunctions.push_back(it->getName().str());
    	if(fpFunctions.back().substr(0,5) == "int32"
    			|| fpFunctions.back().substr(0,5) == "int64"
    			|| fpFunctions.back().substr(0,7) == "float32"
    			|| fpFunctions.back().substr(0,7) == "float64"){
    		std::string tmp = fpFunctions.back();
    		fpStapleFunctions.push_back(tmp);
    	}
    }
    fpFunctions.push_back("estimateDiv128To64");
    fpStapleFunctions.push_back("estimateDiv128To64");
    fpCmpFunctions.push_back("float64_le");
    fpCmpFunctions.push_back("float64_eq");
    fpCmpFunctions.push_back("float64_lt");
    fpCmpFunctions.push_back("float64_gt");
    fpCmpFunctions.push_back("float64_ge");
    fpCmpFunctions.push_back("float64_ne");
